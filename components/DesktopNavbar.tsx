...
const hacktheboxIcon = (
  <svg
    ...
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
    id="Hackthebox--Streamline-Simple-Icons"
    height="24"
    width="24"
  >
    <desc>Hackthebox Streamline Icon: https://streamlinehq.com</desc>
...
export default function DesktopNavbar() {
  ...
  const socialLinks = [
    ...
    },
    {
      name: "Hack The Box",
      icon: hacktheboxIcon,
      href: "https://app.hackthebox.com/public/users/2169585",
    },
  ];
  ...
  return (
    <aside className="w-14 h-full bg-[--color-accent1] flex flex-col items-center justify-between shrink-0">
      <nav className="relative w-full">
        <div
          className="absolute left-0 w-full h-14 transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] z-0"
          style={{
        ...
        <ul className="relative z-10">
          {navItems.map((item) => (
            <li
              key={item.name}
              className={`group relative transition-all duration-300 hover:bg-[var-(--color-secondary)] cursor-default`}
            >
              <Link
                href={item.href}
                className={`flex items-center justify-center w-full h-14 transition-all duration-300 cursor-default ${
                  pathname === item.href
          ...
          {socialLinks.map((social) => (
            <li key={social.name}>
              <a
                ...
                className="flex items-center justify-center w-full h-14 hover:bg-(--color-secondary) text-(--color-foreground) transition-all duration-300 cursor-default"
                title={social.name}
              >
                {social.icon}
              </a>
